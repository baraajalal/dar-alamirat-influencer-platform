...
export async function changeForcedStaffPassword(formData: FormData) {
  ...
  if (!currentPassword) redirect("/staff/change-password?error=current_required");
  if (!passwordIsStrong(newPassword)) redirect("/staff/change-password?error=password_weak");
  if (newPassword !== confirmation) redirect("/staff/change-password?error=password_mismatch");
  if (currentPassword === newPassword) redirect("/staff/change-password?error=password_same");
  ...
  if (!user || !user.email) redirect("/staff/login?error=invalid_credentials");
  ...
  if (verifyError) redirect("/staff/change-password?error=current_invalid");

  const { error: passwordError } = await supabase.auth.updateUser({ password: newPassword });
  if (passwordError) {
    console.error("STAFF_FORCED_PASSWORD_UPDATE_FAILED", passwordError);
    redirect("/staff/change-password?error=update_failed");
  }

  const { data: authResult, error: authLookupError } = await admin.auth.admin.getUserById(user.id);
  if (authLookupError || !authResult.user) {
    console.error("STAFF_FORCED_PASSWORD_METADATA_LOOKUP_FAILED", authLookupError);
