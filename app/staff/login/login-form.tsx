...
export default function StaffLoginForm({ error, success }: { error?: string; success?: string }) {
  ...
  return (
    <main dir="rtl" className="relative min-h-screen overflow-hidden bg-[#F4F6FF] px-4 py-8 font-['Tajawal',Tahoma,Arial,sans-serif] sm:px-8 lg:flex lg:items-center lg:justify-center">
      <div aria-hidden="true" className="absolute -right-28 -top-28 h-96 w-96 rounded-full bg-[#DCE3FF]/80 blur-3xl" />
      ...
      <section className="relative mx-auto grid w-full max-w-6xl overflow-hidden rounded-[34px] border border-white/80 bg-white shadow-[0_30px_90px_rgba(71,88,160,0.18)] lg:grid-cols-[0.88fr_1.12fr]">
        <aside className="relative hidden min-h-[680px] overflow-hidden bg-[linear-gradient(150deg,#6E7FD1_0%,#5063BA_58%,#4053A8_100%)] p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="flex min-h-[650px] items-center justify-center p-6 sm:p-10 lg:p-14">
          <div className="w-full max-w-md">
            <div className="mb-8 text-center">
              <div className="mx-auto mb-5 flex h-24 w-24 items-center justify-center rounded-[28px] bg-[linear-gradient(145deg,#7180D2,#5668BC)] shadow-[0_22px_45px_rgba(86,104,188,0.28)] ring-8 ring-[#EDF0FF]">
                <img src="/da-logo.png" alt="دار الأميرات" className="h-20 w-20 object-contain" />
              </div>
              <p className="text-sm font-extrabold text-[#6877C8]">بوابة فريق دار الأميرات</p>
              <h2 className="mt-2 text-3xl font-black text-[#2F4075]">تسجيل دخول الموظف</h2>
              <p className="mt-3 text-sm font-medium leading-7 text-[#8189A8]">استخدم البريد الإلكتروني الذي استلمت عليه دعوة النظام.</p>
            </div>
            ...
            <form action={staffLogin} className="space-y-4">
              <label className="relative block">
                <span className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-[#6877C8]"><MailIcon /></span>
                <input name="email" type="email" required autoComplete="email" placeholder="البريد الإلكتروني" className="h-16 w-full rounded-2xl border border-[#D8DDF7] bg-[#FBFCFF] pr-12 pl-4 text-sm font-bold text-[#33447F] outline-none transition placeholder:text-[#A1A8C3] focus:border-[#6877C8] focus:bg-white focus:shadow-[0_0_0_4px_rgba(104,119,200,0.12)]" dir="ltr" />
              </label>

              <label className="relative block">
                <span className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-[#6877C8]"><LockIcon /></span>
                <input name="password" type={showPassword ? "text" : "password"} required autoComplete="current-password" placeholder="كلمة المرور" className="h-16 w-full rounded-2xl border border-[#D8DDF7] bg-[#FBFCFF] pr-12 pl-12 text-sm font-bold text-[#33447F] outline-none transition placeholder:text-[#A1A8C3] focus:border-[#6877C8] focus:bg-white focus:shadow-[0_0_0_4px_rgba(104,119,200,0.12)]" dir="ltr" />
                <button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute left-4 top-1/2 -translate-y-1/2 rounded-lg p-1 text-[#7884B8] hover:bg-[#EEF0FB]" aria-label={showPassword ? "إخفاء كلمة المرور" : "إظهار كلمة المرور"}><EyeIcon open={showPassword} /></button>
              </label>

              <div className="flex items-center justify-between gap-4 px-1 text-sm">
                <label className="flex items-center gap-2 font-bold text-[#68708F]"><input type="checkbox" name="remember" className="h-4 w-4 rounded border-[#BBC3E8] accent-[#6877C8]" />تذكرني</label>
                <span className="font-bold text-[#8A92B0]">نسيت كلمة المرور؟ تواصل مع الإدارة</span>
              </div>

              <button type="submit" className="relative h-16 w-full overflow-hidden rounded-2xl bg-[linear-gradient(135deg,#6575CB_0%,#4F60B6_100%)] px-6 text-base font-black text-white shadow-[0_18px_35px_rgba(79,96,182,0.30)] transition hover:-translate-y-0.5 hover:shadow-[0_22px_42px_rgba(79,96,182,0.36)] focus:outline-none focus:ring-4 focus:ring-[#A9B9E6]/55">
                تسجيل الدخول
              </button>
            </form>

            <div className="mt-7 rounded-2xl bg-[#F6F7FF] px-4 py-3 text-center text-xs font-bold leading-6 text-[#7B84A3]">هذه الصفحة مخصصة للموظفين والإدارة فقط. دخول المؤثرين يبقى من بوابة المؤثر الحالية.</div>
          </div>
        </div>
      </section>
    </main>
