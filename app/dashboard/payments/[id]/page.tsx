    ...
    <main className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="rounded-full bg-[#EEF1FF] px-4 py-2 text-xs font-black text-[#5669BE]">{typeText[paymentRow.type as keyof typeof typeText] ?? paymentRow.type}</span>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <Panel title={t.influencer}>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <Link href={`/dashboard/influencers/${assignmentRow.influencer_id}`} className="rounded-xl bg-[#EEF1FF] px-3 py-2 text-xs font-black text-[#586AC1]">{t.influencer}</Link>
              <Link href={`/dashboard/campaigns/${assignmentRow.campaign_id}`} className="rounded-xl bg-[#EEF1FF] px-3 py-2 text-xs font-black text-[#586AC1]">{t.campaign}</Link>
            </div>
          </Panel>
          ...
          {paymentRow.type === "bank_transfer" && canViewFinance ? <Panel title={t.history}>{transactions.length ? <div className="space-y-3">{transactions.map((transaction) => <article key={transaction.id} className="rounded-2xl border border-[#E5E9F5] bg-[#FAFBFF] p-4"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><p className="text-lg font-black text-emerald-700">{money.format(num(transaction.amount))}</p><p className="mt-1 text-xs font-bold text-[#8E97AF]">{date.format(new Date(transaction.transferred_at))}</p></div><div className="text-sm font-bold text-[#53618D]"><p>{transaction.transfer_reference}</p><p className="mt-1 text-xs text-[#9AA1B4]">{transaction.finance_batch_number || "—"}</p></div></div>{transaction.proofUrl ? <a href={transaction.proofUrl} target="_blank" rel="noreferrer" className="mt-3 inline-block text-xs font-black text-[#576AC2]">{t.proofLink} ↗</a> : null}</article>)}</div> : <Empty text={t.noTransactions} />}</Panel> : null}
        </div>

        <aside className="space-y-6">
          <Panel title={t.workflow}>
            <div className="space-y-4">
              {paymentRow.status === "draft" && canSubmit ? <form action={submitPaymentForApproval}><input type="hidden" name="payment_id" value={id} /><button className="w-full rounded-2xl bg-[#596CC5] px-4 py-3.5 text-sm font-black text-white shadow-[0_12px_28px_rgba(75,94,184,.22)]">{t.submit}</button></form> : null}
          ...
          {paymentRow.finance_notes ? <Panel title={t.systemNotes}><p className="text-sm font-bold leading-7 text-[#657096]">{paymentRow.finance_notes}</p></Panel> : null}
        </aside>
      </section>
    </main>
  );
...
function Panel({ title, children }: { title: string; children: React.ReactNode }) { return <section className="rounded-[26px] border border-white/90 bg-white/94 p-5 shadow-[0_18px_55px_rgba(69,83,151,.08)] sm:p-6"><h2 className="mb-5 text-lg font-black text-[#34457E]">{title}</h2>{children}</section>; }
...
function Empty({ text }: { text: string }) { return <div className="rounded-2xl border border-dashed border-[#DDE2F3] bg-[#FAFBFF] p-6 text-center text-sm font-bold text-[#929AB1]">{text}</div>; }
...
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="block"><span className="mb-1.5 block text-xs font-black text-[#69749B]">{label}</span><div className="[&_input]:h-11 [&_input]:w-full [&_input]:rounded-xl [&_input]:border [&_input]:border-[#DFE4F2] [&_input]:bg-[#FAFBFF] [&_input]:px-3 [&_input]:text-sm [&_input]:font-bold [&_input]:text-[#3D4E83] [&_select]:h-11 [&_select]:w-full [&_select]:rounded-xl [&_select]:border [&_select]:border-[#DFE4F2] [&_select]:bg-[#FAFBFF] [&_select]:px-3 [&_select]:text-sm [&_select]:font-bold [&_select]:text-[#3D4E83] [&_textarea]:w-full [&_textarea]:rounded-xl [&_textarea]:border [&_textarea]:border-[#DFE4F2] [&_textarea]:bg-[#FAFBFF] [&_textarea]:p-3 [&_textarea]:text-sm [&_textarea]:font-bold [&_textarea]:text-[#3D4E83]">{children}</div></label>; }
function ReviewForm({ paymentId, decision, label, reasonLabel, tone }: { paymentId: string; decision: "return" | "reject"; label: string; reasonLabel: string; tone: "amber" | "rose" }) { const button = tone === "amber" ? "bg-amber-500 hover:bg-amber-600" : "bg-rose-600 hover:bg-rose-700"; return <form action={reviewPayment} className="rounded-2xl border border-[#E6E9F4] bg-[#FAFBFF] p-3"><input type="hidden" name="payment_id" value={paymentId} /><input type="hidden" name="decision" value={decision} /><textarea name="reason" required rows={2} placeholder={reasonLabel} className="w-full rounded-xl border border-[#DFE4F2] bg-white p-3 text-sm font-bold text-[#3D4E83] outline-none" /><button className={`mt-2 w-full rounded-xl px-4 py-3 text-sm font-black text-white transition ${button}`}>{label}</button></form>; }
