        ...
        <div className="space-y-6">
          ...
          <Panel title={t.campaigns}>
            <div className="space-y-3">
              {assignmentRows.map((assignment) => { const campaign = campaignMap.get(assignment.campaign_id); return <Link key={assignment.id} href={`/dashboard/campaigns/${assignment.campaign_id}`} className="flex flex-col justify-between gap-3 rounded-2xl border border-[#F2ECF6] bg-[#FDFBFE] p-4 transition hover:border-[#BFC9ED] sm:flex-row sm:items-center"><div><p className="font-black text-[#432A57]">{campaign?.name ?? "حملة"}</p><p className="mt-1 text-xs font-bold text-[#9299AE]">{campaign?.brand ?? "—"} · {assignment.execution_type ?? "—"}</p></div><div className="flex items-center gap-3"><span className="rounded-full bg-[#F7F0FA] px-3 py-1 text-xs font-black text-[#915FA9]">{assignmentLabels[assignment.status] ?? assignment.status}</span><span className="text-sm font-black text-[#455791]">{money.format(Number(assignment.agreed_amount ?? 0))}</span></div></Link>; })}
