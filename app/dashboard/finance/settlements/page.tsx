...
function settlementLabel(row: { fully_settled: boolean | null; pending_approval_count: number; pending_execution_count: number; zero_amount_count: number }) {
  ...
  if (row.pending_execution_count > 0) return { label: "قيد التنفيذ", className: "bg-[#EEF1FF] text-[#596BC4]" };
...
export default async function SettlementsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <div dir="rtl" className="space-y-6">
      ...
      <section className="rounded-[26px] border border-[#E1E6F5] bg-white p-5 shadow-[0_14px_42px_rgba(67,82,155,.07)]">
        <form className="flex flex-wrap items-end gap-3">
          <label className="min-w-[260px] flex-1 text-xs font-black text-[#69769C]">بحث
          ...
          <label className="min-w-[210px] text-xs font-black text-[#69769C]">حالة التسوية
          ...
          <button className="rounded-2xl bg-[#5368C3] px-6 py-3 text-sm font-black text-white">تطبيق</button>
        </form>
      </section>
        ...
        <table className="min-w-[1180px] w-full text-right text-sm">
          ...
          <tbody className="divide-y divide-[#EEF0F7]">
            {rows.map((row) => {
              ...
              return <tr key={row.assignment_id} className="text-[#485985]">
                <td className="px-4 py-4"><p className="font-black">{row.influencer_name}</p><p className="mt-1 text-xs text-[#8A93AE]" dir="ltr">{row.mobile}</p></td>
                <td className="px-4 py-4"><p className="font-black">{row.campaign_name}</p><p className="mt-1 text-xs text-[#8A93AE]">{row.brand_name ?? "—"}</p></td>
                ...
                <td className="px-4 py-4 text-xs font-bold"><p>{date(row.settled_at)}</p>{row.availability_blocked_until ? <><p className="mt-1 text-[#8A93AE]">الحظر حتى {date(row.availability_blocked_until)}</p><p className="mt-1 font-black text-amber-700">متبقي {remainingDays(row.availability_blocked_until)} يوم</p></> : null}</td>
                <td className="px-4 py-4"><form action={refreshSettlement}><input type="hidden" name="assignment_id" value={row.assignment_id} /><button className="rounded-xl bg-[#EEF1FF] px-3 py-2 text-xs font-black text-[#596BC4]">مراجعة وإقفال</button></form></td>
              </tr>;
            })}
          </tbody>
        </table>
        {rows.length === 0 ? <div className="p-10 text-center text-sm font-black text-[#8A93AE]">لا توجد نتائج مطابقة.</div> : null}
      </section>
    </div>
  );
}
