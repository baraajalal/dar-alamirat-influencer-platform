      ...
      <section className="rounded-[28px] border border-[#F0E7F4] bg-white p-6 shadow-[0_16px_48px_rgba(67,82,155,.07)]">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <h2 className="mt-1 text-xl font-black text-[#4F3762]">آخر مجموعات التحويل</h2>
          </div>
          <Link href="/dashboard/finance/transfers" className="rounded-xl bg-[#F7F0FA] px-4 py-2 text-xs font-black text-[#9362AD]">فتح التحويلات</Link>
        </div>
        ...
        <div className="mt-5 overflow-x-auto rounded-2xl border border-[#F1EAF5]">
            ...
            <tbody className="divide-y divide-[#F5EFF7]">
              {(batches ?? []).slice(0, 20).map((batch) => (
                <tr key={batch.id} className="text-[#604970]">
                  <td className="px-4 py-4"><Link href={`/dashboard/finance/transfers/${batch.id}`} className="font-black text-[#9362AD]">{batch.name}</Link><p className="mt-1 text-xs text-[#96859E]" dir="ltr">{batch.batch_code}</p></td>
          ...
          {(batches ?? []).length === 0 ? <div className="p-8 text-center text-sm font-black text-[#8D7B95]">لا توجد مجموعات تحويل بعد.</div> : null}
        </div>
      </section>
    </div>
  );
