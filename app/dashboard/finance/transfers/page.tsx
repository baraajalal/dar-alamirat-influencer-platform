    ...
    <div dir="inherit" className="space-y-6">
      ...
      <section className="rounded-[28px] border border-[#F0E7F4] bg-white p-6 shadow-[0_16px_48px_rgba(67,82,155,.07)]">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-xs font-black text-[#8D7B95]">جاهزة للتجميع</p>
            <h2 className="mt-1 text-xl font-black text-[#4F3762]">إنشاء مسودة تحويل جديدة</h2>
          </div>
        ...
        <form action={createPaymentBatch} className="mt-5 space-y-5">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <label className="text-xs font-black text-[#705E7B]">اسم المجموعة<input name="name" required placeholder="تحويلات الأسبوع الأول" className="mt-2 w-full rounded-xl border border-[#ECE1F1] px-4 py-3 text-sm font-bold outline-none" /></label>
            <label className="text-xs font-black text-[#705E7B]">رقم المجموعة<input name="batch_code" required placeholder="AUG-2026-W1" dir="ltr" className="mt-2 w-full rounded-xl border border-[#ECE1F1] px-4 py-3 text-sm font-bold outline-none" /></label>
            <label className="text-xs font-black text-[#705E7B]">الشهر التابع للمجموعة<input name="month_label" required placeholder="أغسطس 2026" className="mt-2 w-full rounded-xl border border-[#ECE1F1] px-4 py-3 text-sm font-bold outline-none" /></label>
            <label className="text-xs font-black text-[#705E7B]">تاريخ التحويل المتوقع<input name="scheduled_for" type="date" className="mt-2 w-full rounded-xl border border-[#ECE1F1] px-4 py-3 text-sm font-bold outline-none" /></label>
          </div>

          <div className="overflow-x-auto rounded-2xl border border-[#F1EAF5]">
            <table className="min-w-full text-right text-sm">
              <thead className="bg-[#FAF6FC] text-xs font-black text-[#715F7D]">
                <tr>
              ...
              <tbody className="divide-y divide-[#F5EFF7]">
                {readyPayments.map((payment) => {
                  ...
                  return (
                    <tr key={payment.id} className="text-[#604970]">
                      <td className="px-4 py-4"><input type="checkbox" name="payment_ids" value={payment.id} className="h-4 w-4" /></td>
                      <td className="px-4 py-4"><p className="font-black">{influencer?.full_name ?? "—"}</p><p className="mt-1 text-xs text-[#96859E]" dir="ltr">{influencer?.mobile_e164 ?? "—"}</p></td>
                      <td className="px-4 py-4"><p className="font-black">{campaign?.name ?? "—"}</p><p className="mt-1 text-xs text-[#96859E]">{campaign?.brand ?? "—"}</p></td>
            ...
            {readyPayments.length === 0 ? <div className="p-8 text-center text-sm font-black text-[#8D7B95]">لا توجد مستحقات معتمدة جاهزة للتجميع.</div> : null}
          </div>

          <button disabled={readyPayments.length === 0} className="rounded-xl bg-[#9362AD] px-6 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-50">حفظ كمسودة جاهزة للمراجعة</button>
        </form>
      </section>
      ...
      <section className="rounded-[28px] border border-[#F0E7F4] bg-white p-6 shadow-[0_16px_48px_rgba(67,82,155,.07)]">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-xs font-black text-[#8D7B95]">المجموعات المالية</p>
            <h2 className="mt-1 text-xl font-black text-[#4F3762]">مسودات ومجموعات التحويل</h2>
          </div>
          <span className="rounded-full bg-[#F7F0FA] px-4 py-2 text-sm font-black text-[#9362AD]">{batches?.length ?? 0}</span>
        </div>

        <div className="mt-5 overflow-x-auto rounded-2xl border border-[#F1EAF5]">
          <table className="min-w-full text-right text-sm">
            <thead className="bg-[#FAF6FC] text-xs font-black text-[#715F7D]"><tr><th className="px-4 py-4">المجموعة</th><th className="px-4 py-4">الشهر</th><th className="px-4 py-4">الإجمالي</th><th className="px-4 py-4">البنك / يدوي</th><th className="px-4 py-4">الحالة</th><th className="px-4 py-4">الإجراء</th></tr></thead>
            <tbody className="divide-y divide-[#F5EFF7]">
              {(batches ?? []).map((batch) => (
                <tr key={batch.id} className="text-[#604970]">
                  <td className="px-4 py-4"><p className="font-black">{batch.name}</p><p className="mt-1 text-xs text-[#96859E]" dir="ltr">{batch.batch_code}</p></td>
                  ...
                  <td className="px-4 py-4"><span className="rounded-full bg-[#F8F2FB] px-3 py-1.5 text-xs font-black text-[#9362AD]">{statusLabel(batch.status)}</span></td>
                  <td className="px-4 py-4"><Link href={`/dashboard/finance/transfers/${batch.id}`} className="rounded-xl bg-[#F7F0FA] px-4 py-2 text-xs font-black text-[#9362AD]">فتح المسودة</Link></td>
                </tr>
              ))}
            </tbody>
          </table>
          {(batches ?? []).length === 0 ? <div className="p-8 text-center text-sm font-black text-[#8D7B95]">لم يتم إنشاء مجموعات تحويل بعد.</div> : null}
        </div>
      </section>
    </div>
  );
