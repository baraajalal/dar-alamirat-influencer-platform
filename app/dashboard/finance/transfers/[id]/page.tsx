    ...
    <div dir="inherit" className="space-y-6">
      <section className="rounded-[30px] bg-[linear-gradient(135deg,#9566AF,#C5A2D5)] p-7 text-white shadow-[0_24px_64px_rgba(70,86,180,.22)]">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link href="/dashboard/finance/transfers" className="rounded-2xl bg-white px-5 py-3 text-sm font-black text-[#9566AF]">العودة للمجموعات</Link>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-[28px] border border-[#F0E7F4] bg-white p-6 shadow-[0_16px_48px_rgba(67,82,155,.07)]">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-xs font-black text-[#8D7B95]">تفاصيل المسودة</p>
            <h2 className="mt-1 text-xl font-black text-[#4F3762]">المستفيدون والتحويلات</h2>
          </div>
          {canExport ? (
            <div className="flex flex-wrap gap-2">
              <a href={`/api/finance/payment-batches/${batch.id}/export?type=review`} className="rounded-xl bg-[#F7F0FA] px-4 py-2 text-xs font-black text-[#9362AD]">Excel المراجعة</a>
        ...
        <div className="mt-5 overflow-x-auto rounded-2xl border border-[#F1EAF5]">
          <table className="min-w-[1300px] text-right text-sm">
            ...
            <tbody className="divide-y divide-[#F5EFF7]">
              {(items ?? []).map((item) => (
                <tr key={item.id} className="text-[#604970]">
                  <td className="px-4 py-4"><p className="font-black">{item.influencer_name}</p><p className="mt-1 text-xs text-[#96859E]" dir="ltr">{item.mobile}</p></td>
                  <td className="px-4 py-4"><p className="font-black">{item.campaign_name}</p><p className="mt-1 text-xs text-[#96859E]">{item.brand_name ?? "—"}</p></td>
                  <td className="px-4 py-4 font-black">{money(num(item.amount))}</td>
                  <td className="px-4 py-4"><p className="font-bold">{item.bank_name ?? "—"}</p><p className="mt-1 text-xs text-[#96859E]">{item.account_holder_name ?? "—"}</p></td>
                  <td className="px-4 py-4 font-bold" dir="ltr">{item.iban ?? "—"}</td>
                  <td className="px-4 py-4"><p className="font-bold">{item.identity_number ?? "غير متوفر"}</p><p className="mt-1 text-xs text-[#96859E]">{item.identity_type ?? "—"}</p></td>
                  ...
                  <td className="px-4 py-4">{item.publication_url ? <a href={item.publication_url} target="_blank" rel="noreferrer" className="text-xs font-black text-[#9362AD]">فتح الرابط</a> : <span className="text-xs font-bold text-[#96859E]">دفع مسبق/لا يوجد</span>}</td>
                  ...
                  <td className="px-4 py-4"><p className="font-black">{item.item_status === "failed" ? "فشل التحويل" : item.item_status}</p>{item.proof_path ? <a href={item.proof_path} target="_blank" rel="noreferrer" className="mt-2 inline-flex text-xs font-black text-[#9362AD]">فتح الإثبات</a> : <form action={saveTransferProof} className="mt-2 space-y-2"><input type="hidden" name="batch_id" value={batch.id}/><input type="hidden" name="item_id" value={item.id}/><input name="proof_path" placeholder="رابط/مسار الإثبات" className="w-44 rounded-lg border border-[#ECE1F1] p-2 text-xs"/><input name="transfer_reference" placeholder="مرجع التحويل" className="w-44 rounded-lg border border-[#ECE1F1] p-2 text-xs"/><button className="block rounded-lg bg-[#F7F0FA] px-3 py-2 text-xs font-black text-[#9362AD]">حفظ الإثبات</button></form>}</td>
                  <td className="px-4 py-4">{item.item_status === "failed" ? <div className="max-w-xs"><p className="font-black text-rose-700">{item.failure_reason}</p><p className="mt-1 text-xs">{item.bank_response_reference ?? "—"}</p></div> : batch.status === "processing" || batch.status === "submitted_to_bank" ? <form action={markTransferItemFailed} className="w-56 space-y-2"><input type="hidden" name="batch_id" value={batch.id}/><input type="hidden" name="item_id" value={item.id}/><select name="failure_category" className="w-full rounded-lg border border-[#ECE1F1] p-2 text-xs font-bold"><option value="">تصنيف الفشل</option><option value="bank_data">بيانات بنكية</option><option value="beneficiary_rejected">رفض المستفيد</option><option value="bank_rejected">رفض البنك</option><option value="technical">مشكلة تقنية</option><option value="other">أخرى</option></select><input name="bank_response_reference" placeholder="مرجع البنك" className="w-full rounded-lg border border-[#ECE1F1] p-2 text-xs"/><textarea name="failure_reason" placeholder="سبب الفشل" rows={2} className="w-full rounded-lg border border-[#ECE1F1] p-2 text-xs"/><button className="rounded-lg bg-rose-600 px-3 py-2 text-xs font-black text-white">تسجيل كمرتجع</button></form> : <span className="text-xs font-bold text-[#96859E]">بعد الرفع للبنك</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        <article className="rounded-[28px] border border-[#F0E7F4] bg-white p-6 shadow-[0_16px_48px_rgba(67,82,155,.07)]">
          <h2 className="text-xl font-black text-[#4F3762]">مراجعة واعتماد المسودة</h2>
          {batch.status === "draft" || batch.status === "returned" ? (
            <form action={submitBatchForReview} className="mt-5">
              <input type="hidden" name="batch_id" value={batch.id} />
              <button className="w-full rounded-xl bg-[#9362AD] px-5 py-3 text-sm font-black text-white">إرسال المسودة للمراجعة</button>
            </form>
          ) : null}
          ...
          {batch.status === "under_review" ? (
            <form action={reviewPaymentBatch} className="mt-5 space-y-3">
              <input type="hidden" name="batch_id" value={batch.id} />
              <textarea name="notes" rows={3} placeholder="ملاحظة الإرجاع أو الرفض" className="w-full rounded-xl border border-[#ECE1F1] p-3 text-sm font-bold outline-none" />
        ...
        <article className="rounded-[28px] border border-[#F0E7F4] bg-white p-6 shadow-[0_16px_48px_rgba(67,82,155,.07)]">
          <h2 className="text-xl font-black text-[#4F3762]">حالة تنفيذ المجموعة</h2>
          <p className="mt-2 text-sm font-bold leading-7 text-[#82718C]">بعد تجهيز الملفات، حدّثي حالة الرفع للبنك والتنفيذ. الإكمال يغلق المستحقات داخل المجموعة.</p>
          <form action={updateBatchExecutionStatus} className="mt-5 grid gap-2 sm:grid-cols-2">
            <input type="hidden" name="batch_id" value={batch.id} />
            <button disabled={!canExport} name="action" value="mark_exported" className="rounded-xl bg-[#F7F0FA] px-4 py-3 text-sm font-black text-[#9362AD] disabled:opacity-40">تم تجهيز الملفات</button>
...
function Info({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl bg-[#FBF8FD] p-4"><p className="text-xs font-black text-[#95849D]">{label}</p><p className="mt-2 text-sm font-black text-[#4B5B87]">{value}</p></div>;
}
