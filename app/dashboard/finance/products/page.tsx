...
export default async function ProductsPage({ searchParams }: { searchParams?: Promise<{ status?: string; success?: string }> }) {
  ...
  return <div dir="rtl" className="space-y-6">
    ...
    <form className="flex flex-wrap gap-2 rounded-2xl bg-white p-4 shadow-sm"><select name="status" defaultValue={query.status ?? ""} className="rounded-xl border border-[#DDE2F2] px-4 py-2 text-sm font-bold"><option value="">كل الحالات</option>{Object.entries(statusLabels).map(([v,l]) => <option key={v} value={v}>{l}</option>)}</select><button className="rounded-xl bg-[#527B70] px-4 py-2 text-sm font-black text-white">تطبيق الفلتر</button></form>
    <section className="grid gap-5 xl:grid-cols-2">{(rows ?? []).map(row => { const a=assignmentMap.get(row.assignment_id); const i=a ? influencerMap.get(a.influencer_id) : null; const c=a ? campaignMap.get(a.campaign_id) : null; return <article key={row.id} className="rounded-[26px] border border-[#E1E8E5] bg-white p-5 shadow-[0_14px_40px_rgba(67,82,155,.06)]"><div className="flex items-start justify-between gap-4"><div><h2 className="text-lg font-black text-[#3D5F56]">{i?.full_name ?? "—"}</h2><p className="mt-1 text-xs font-bold text-[#8A9B96]" dir="ltr">{i?.mobile_e164 ?? "—"}</p><p className="mt-3 text-sm font-black text-[#566B66]">{c?.name ?? "—"} · {c?.brand ?? "—"}</p></div><div className="text-left"><span className="rounded-full bg-[#EDF6F2] px-3 py-1.5 text-xs font-black text-[#527B70]">{statusLabels[row.status] ?? row.status}</span><p className="mt-3 text-sm font-black text-[#3D5F56]">{money(amountMap.get(row.payment_id) ?? 0)}</p></div></div><form action={updateProductFulfilment} className="mt-5 grid gap-3 md:grid-cols-2"><input type="hidden" name="id" value={row.id}/><select name="status" defaultValue={row.status} className="rounded-xl border border-[#DDE2F2] p-3 text-sm font-bold">{Object.entries(statusLabels).map(([v,l]) => <option key={v} value={v}>{l}</option>)}</select><input name="order_number" defaultValue={row.order_number ?? ""} placeholder="رقم الطلب" className="rounded-xl border border-[#DDE2F2] p-3 text-sm"/><input name="carrier_name" defaultValue={row.carrier_name ?? ""} placeholder="شركة الشحن" className="rounded-xl border border-[#DDE2F2] p-3 text-sm"/><input name="tracking_number" defaultValue={row.tracking_number ?? ""} placeholder="رقم التتبع" className="rounded-xl border border-[#DDE2F2] p-3 text-sm"/><input name="proof_path" defaultValue={row.proof_path ?? ""} placeholder="رابط/مسار إثبات التسليم" className="rounded-xl border border-[#DDE2F2] p-3 text-sm md:col-span-2"/><textarea name="notes" defaultValue={row.notes ?? ""} placeholder="ملاحظات" rows={2} className="rounded-xl border border-[#DDE2F2] p-3 text-sm md:col-span-2"/><button className="rounded-xl bg-[#527B70] px-5 py-3 text-sm font-black text-white md:col-span-2">حفظ التحديث</button></form></article>})}</section>
    {(rows ?? []).length === 0 ? <div className="rounded-2xl bg-white p-10 text-center text-sm font-black text-[#8A93AE]">لا توجد مستحقات منتجات بهذه الحالة.</div> : null}
  </div>;
}
