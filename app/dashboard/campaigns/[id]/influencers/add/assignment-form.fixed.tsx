...
export default function AssignmentForm({
  ...
  useEffect(() => {
    if (executionType === "in_branch" && voucherSource === "branch" && branchName.trim() && !voucherBranch.trim()) {
      setVoucherBranch(branchName.trim());
    }
  }, [executionType, voucherSource, branchName, voucherBranch]);
    ...
    <form action={formAction} onSubmit={prepareSubmission} className="space-y-6" ref={topRef}>
                      ...
                      <div key={account.id} className={`rounded-[22px] border p-5 transition ${selected ? "border-[#D8BDE3] bg-[#F7F8FF]" : "border-[#F1EAF5] bg-[#FDFBFE]"}`}>
                        ...
                        {selected && effectiveRequiresContent ? (
                          <div className="mt-5 space-y-3 border-t border-[#E1E5F2] pt-5">
                            ...
                            <p className="text-xs font-semibold leading-6 text-[#8D7B95]">{t.quantityHint}</p>
                          </div>
                        ) : null}
                      </div>
                    );
      ...
      {step === 3 ? (
        <CampaignPanel title={t.step3Title} description={t.step3Subtitle}>
          ...
          <div className="mt-4 rounded-2xl border border-[#E8D9EE] bg-[#FBF7FD] px-5 py-4 text-xs font-bold leading-7 text-[#75677B]">{t.compensationMixHint}</div>
...
function text(locale: CampaignLocale) {
  if (locale === "en") return {
    ...
    step3Title: "3. Compensation, payment and contract", step3Subtitle: "Select one or more compensation types. Each selected type has its own amount and details.", bankTransfer: "Advertising fee / bank transfer", bankHint: "A cash advertising fee processed by finance. It can be combined with a branch voucher or products.", voucher: "Shopping voucher", voucherHint: "Redeemed online or at a selected branch.", products: "Products", productsHint: "Cash value is zero while product value is kept for reporting.", noCompensation: "No compensation was selected. The assignment will be saved as unpaid.", bankBadge: "Ad fee", voucherBadge: "Voucher", productsBadge: "Products", bankAmount: "Advertising fee amount", expectedPayment: "Expected payment date", bankNotes: "Advertising fee notes", financeNotesPlaceholder: "Any information required by finance...", voucherAmount: "Voucher value", voucherSource: "Voucher redemption", website: "Online store", aBranch: "A branch", voucherBranch: "Voucher branch", voucherNotes: "Voucher notes", voucherNotesPlaceholder: "Voucher code or delivery method when available...", cashValue: "Cash payment value", productReference: "Product value for reporting", productDescription: "Product description", productDescriptionPlaceholder: "List the products or package provided to the influencer...", productNotes: "Product delivery notes", productNotesPlaceholder: "Preparation date or delivery method...", estimatedBudget: "Estimated budget", currentCommitted: "Currently committed", remainingBefore: "Remaining before", remainingAfter: "Remaining after", budgetRule: "Only bank transfers and vouchers count against the influencer campaign budget. Home order and product values are reported separately.", overBudget: "The estimated budget will be exceeded by", warningOnly: "This is a warning and does not prevent saving.", withinBudget: "Compensation is within budget. Remaining after adding:", hasContract: "This collaboration is governed by a contract or agreement", contractHint: "Finance will see the contract flag and payment timing clearly.", contractReference: "Contract or agreement reference", agreementDate: "Agreement date", paymentTiming: "Payment timing", beforePublish: "Before publishing", afterPublish: "After publishing", byAgreement: "By agreement", contractNotes: "Contract notes", coordinatorNotes: "Coordinator notes", coordinatorNotesPlaceholder: "Internal coordination notes for this influencer...", summary: "Assignment summary", influencer: "Influencer", platforms: "Platforms", collaboration: "Collaboration", financialTotal: "Financial total", compensationMixHint: "The collaboration method and compensation are separate. For example, choose In-branch collaboration, then select both a branch voucher and an advertising fee if that is the agreement.", quantityHint: "The quantity is the number of separate content pieces. If you set 2 videos, the creator will see two separate publication-link slots and can submit each one independently, even on different dates.",
  } as const;
  return {
