...
export default async function MarketingProjectsComingSoonPage() {
  ...
  return (
    <div dir="inherit" className="space-y-6">
      <section className="rounded-[30px] bg-gradient-to-br from-[#A775C0] to-[#4D61B8] p-8 text-white shadow-[0_24px_65px_rgba(74,88,162,0.22)]">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {["المشاريع والجهات", "الخدمات ونطاق العمل", "المهام والفعاليات", "العقود والفواتير"].map((title) => (
          <div key={title} className="rounded-3xl border border-[#EEE2F2] bg-white p-6 shadow-sm">
            <span className="rounded-full bg-[#F7F0FA] px-3 py-1 text-xs font-black text-[#5D6FC4]">قريبًا</span>
            <h2 className="mt-4 font-black text-[#4A315C]">{title}</h2>
          </div>
        ))}
      </section>
    </div>
