...
export default async function InfluencerNotificationsPage() {
  ...
  return (
    <div dir="inherit" className="space-y-5">
      ...
      {unread > 0 ? (
        <form action={markAllNotificationsRead}>
          <button className="rounded-xl border border-[#ECE1F1] bg-white px-5 py-3 text-sm font-black text-[#9362AD]">تحديد الكل كمقروء</button>
        </form>
      ) : null}

      <section className="space-y-3">
        {(notifications ?? []).map((notification) => (
          <article key={notification.id} className={`rounded-[22px] border p-5 shadow-[0_12px_36px_rgba(67,82,155,.06)] ${notification.read_at ? "border-[#F2ECF5] bg-white" : "border-[#BFC8EF] bg-[#FCF9FD]"}`}>
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <span className="text-xs font-bold text-[#95849D]">{formatDate(notification.created_at)}</span>
            </div>
            {notification.action_url ? <Link href={notification.action_url} className="mt-4 inline-flex rounded-xl bg-[#F7F0FA] px-4 py-2 text-xs font-black text-[#9362AD]">فتح الصفحة المرتبطة</Link> : null}
          </article>
        ))}
        {(notifications ?? []).length === 0 ? <div className="rounded-[24px] border border-dashed border-[#E5D5EC] bg-white p-10 text-center text-sm font-black text-[#8D7B95]">لا توجد إشعارات حتى الآن.</div> : null}
      </section>
    </div>
  );
}
