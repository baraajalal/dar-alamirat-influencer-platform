...
export default function InfluencerOnboardingWizard({ editToken }: { editToken?: string }) {
  ...
  function next() { const error = validateCurrent(); if (error) { showError(error); return; } setStep(s => Math.min(4, s + 1)); window.scrollTo({ top: 0, behavior: "smooth" }); }
  ...
  if (loading) return <main dir="rtl" className="min-h-screen bg-[#F5F7FD] p-10 text-center font-bold text-[#52629B]">جاري تحميل طلبك...</main>;

  return (
    <main dir="rtl" className="min-h-screen bg-[linear-gradient(135deg,#F8F9FE,#EEF1FA)] px-4 py-7 font-['Tajawal',Tahoma,Arial,sans-serif] text-[#33447F]">
      <div className="mx-auto max-w-7xl">
        <header className="mb-6 flex items-center justify-between rounded-[24px] border border-white bg-white/90 px-5 py-4 shadow-sm">
          ...
          <Link href="/login" className="rounded-xl border border-[#D9DEF1] px-4 py-2 text-sm font-black text-[#6575CB]">لدي حساب</Link>
        </header>
        ...
        <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
          <aside className="h-fit rounded-[28px] bg-[linear-gradient(145deg,#6878CA,#5262B8)] p-6 text-white shadow-[0_20px_50px_rgba(79,96,182,.2)] lg:sticky lg:top-6">
          ...
          <section className="rounded-[30px] border border-[#E0E4F2] bg-white p-5 shadow-[0_18px_55px_rgba(67,82,155,.08)] sm:p-8">
            ...
            <div className="mt-8 flex items-center justify-between gap-3 border-t border-[#EEF0F6] pt-6">
              ...
              {step < 4 ? <button type="button" onClick={next} className="rounded-2xl bg-[linear-gradient(135deg,#6575CB,#4F60B6)] px-8 py-3 font-black text-white shadow-lg">التالي</button> : <button type="button" disabled={submitting} onClick={submit} className="rounded-2xl bg-[linear-gradient(135deg,#6575CB,#4F60B6)] px-8 py-3 font-black text-white shadow-lg disabled:opacity-60">{submitting ? "جاري الإرسال..." : editToken ? "إعادة الإرسال للمراجعة" : "إرسال طلب التفعيل"}</button>}
            </div>
          </section>
        </div>
      </div>
...
function Field({ label, value, onChange, type="text", dir }: any) { return <label className="block"><span className="mb-2 block text-sm font-black text-[#4C5985]">{label}</span><input type={type} value={value} onChange={e=>onChange(e.target.value)} dir={dir} className="h-14 w-full rounded-2xl border border-[#D9DEF0] bg-[#FBFCFF] px-4 font-bold outline-none focus:border-[#6877C8] focus:ring-4 focus:ring-[#6877C8]/10" /></label>; }
function PersonalStep({ form, setForm }: any) { return <div><StepTitle title="البيانات الشخصية" desc="أدخل بياناتك الأساسية كما تريد أن تظهر في ملف المؤثر."/><div className="grid gap-4 sm:grid-cols-2"><Field label="الاسم الكامل *" value={form.fullName} onChange={(v:string)=>setForm((f:any)=>({...f,fullName:v}))}/><Field label="رقم الجوال *" value={form.mobile} onChange={(v:string)=>setForm((f:any)=>({...f,mobile:v}))} dir="ltr"/><Field label="البريد الإلكتروني *" type="email" value={form.email} onChange={(v:string)=>setForm((f:any)=>({...f,email:v}))} dir="ltr"/><Field label="سنة الميلاد *" type="number" value={form.birthYear} onChange={(v:string)=>setForm((f:any)=>({...f,birthYear:v}))}/><Field label="المدينة *" value={form.city} onChange={(v:string)=>setForm((f:any)=>({...f,city:v}))}/><Field label="الدولة" value={form.country} onChange={(v:string)=>setForm((f:any)=>({...f,country:v}))}/></div><div className="mt-5"><p className="mb-3 text-sm font-black text-[#4C5985]">الجنس *</p><div className="grid grid-cols-3 gap-3">{[["female","أنثى"],["male","ذكر"],["other","شيء آخر"]].map(([v,l])=><button key={v} type="button" onClick={()=>setForm((f:any)=>({...f,gender:v}))} className={`rounded-2xl border p-4 font-black ${form.gender===v?"border-[#6877C8] bg-[#EEF0FB] text-[#5262B8]":"border-[#D9DEF0]"}`}>{l}</button>)}</div></div></div>; }
function MawthooqStep({ form, setForm }: any) { return <div><StepTitle title="بيانات موثوق" desc="اختر حالة موثوق ثم أضف الرقم والتاريخ إذا كان لديك موثوق."/><div className="grid gap-3 sm:grid-cols-2">{[["yes","لدي موثوق"],["no","لا يوجد لدي موثوق"]].map(([v,l])=><button key={v} type="button" onClick={()=>setForm((f:any)=>({...f,hasMawthooq:v}))} className={`rounded-2xl border p-5 text-right font-black ${form.hasMawthooq===v?"border-[#6877C8] bg-[#EEF0FB]":"border-[#D9DEF0]"}`}>{l}</button>)}</div>{form.hasMawthooq==="yes"?<div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="رقم موثوق *" value={form.mawthooqNumber} onChange={(v:string)=>setForm((f:any)=>({...f,mawthooqNumber:v}))}/><Field label="تاريخ الانتهاء" type="date" value={form.mawthooqExpiryDate} onChange={(v:string)=>setForm((f:any)=>({...f,mawthooqExpiryDate:v}))}/></div>:null}</div>; }
function SocialStep({ rows, update, addOther }: any) { return <div><StepTitle title="حسابات التواصل الاجتماعي" desc="أضف رابط الحساب وعدد المتابعين. المقاييس الإضافية اختيارية وتبقى مرتبة داخل نفس صف المنصة."/><div className="space-y-4">{rows.map((row:SocialRow,i:number)=><div key={`${row.platform}-${i}`} className="rounded-[24px] border border-[#E0E4F2] bg-[#FBFCFF] p-4"><div className="grid gap-3 lg:grid-cols-[130px_1fr_160px_auto]"><div className="flex items-center rounded-xl bg-[#EEF0FB] px-3 font-black text-[#5363AA]">{row.platform==="Other"?<input value={row.otherPlatformName} onChange={e=>update(i,{otherPlatformName:e.target.value})} placeholder="اسم المنصة" className="w-full bg-transparent outline-none"/>:row.platform}</div><input dir="ltr" value={row.profileUrl} onChange={e=>update(i,{profileUrl:e.target.value})} placeholder="رابط الحساب" className="h-12 rounded-xl border border-[#D9DEF0] bg-white px-3 font-bold outline-none"/><input dir="ltr" inputMode="numeric" value={row.followersCount} onChange={e=>update(i,{followersCount:e.target.value})} placeholder="عدد المتابعين" className="h-12 rounded-xl border border-[#D9DEF0] bg-white px-3 font-bold outline-none"/><button type="button" onClick={()=>update(i,{advanced:!row.advanced})} className="rounded-xl border border-[#CBD2EB] px-3 text-xs font-black text-[#6170B8]">{row.advanced?"إخفاء المقاييس":"+ مقاييس أخرى"}</button></div>{row.advanced?<div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[["username","اسم المستخدم"],["averageViews","متوسط المشاهدات"],["averageLikes","متوسط الإعجابات"],["averageComments","متوسط التعليقات"],["engagementRate","نسبة التفاعل %"],["femaleAudience","جمهور إناث %"],["maleAudience","جمهور ذكور %"],["audienceMainCity","مدينة الجمهور"],["audienceMainCountry","دولة الجمهور"]].map(([k,p])=><input key={k} value={(row as any)[k]} onChange={e=>update(i,{[k]:e.target.value})} placeholder={p} className="h-11 rounded-xl border border-[#D9DEF0] bg-white px-3 text-sm font-bold outline-none"/>)}</div>:null}</div>)}</div><button type="button" onClick={addOther} className="mt-5 rounded-2xl border border-dashed border-[#AAB4DE] px-5 py-3 font-black text-[#6575CB]">+ إضافة منصة أخرى</button></div>; }
function CheckGroup({ title, values, selected, onToggle }: any) { return <div><h3 className="mb-3 text-base font-black text-[#44527F]">{title}</h3><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{values.map((v:string)=><label key={v} className={`flex cursor-pointer items-center gap-3 rounded-2xl border p-3 text-sm font-bold ${selected.includes(v)?"border-[#6877C8] bg-[#EEF0FB] text-[#5262B8]":"border-[#E0E4F2]"}`}><input type="checkbox" checked={selected.includes(v)} onChange={()=>onToggle(v)} className="h-4 w-4 accent-[#6575CB]"/>{v}</label>)}</div></div>; }
...
function ReviewStep({ form, social }: any) { const active=social.filter((x:SocialRow)=>x.profileUrl||x.username); return <div><StepTitle title="مراجعة وإرسال" desc="راجع بياناتك قبل إرسالها لفريق دار الأميرات."/><div className="grid gap-4 sm:grid-cols-2"><Summary title="البيانات الشخصية" lines={[form.fullName,form.mobile,form.email,`${form.birthYear} • ${form.city}`]}/><Summary title="موثوق" lines={[form.hasMawthooq==="yes"?`لدي موثوق: ${form.mawthooqNumber}`:"لا يوجد موثوق"]}/><Summary title="حسابات التواصل" lines={active.map((x:SocialRow)=>`${x.platform==="Other"?x.otherPlatformName:x.platform}: ${x.followersCount||"—"} متابع`)}/><Summary title="التفضيلات" lines={[...form.preferredAdCategories,...form.contentStylePreference,...form.shootingStylePreferences]}/></div><div className="mt-5 rounded-2xl bg-[#F6F8FD] p-4 text-sm font-bold leading-7 text-[#65729A]">إرسال الطلب لا يفعّل البوابة مباشرة. سيقوم الموظف بالمراجعة ثم يوافق، يطلب تعديلًا، أو يرفض. لا يتم إرسال بريد إلكتروني؛ رابط التفعيل أو التعديل يرسله الموظف لك بالطريقة المناسبة.</div></div>; }
...
function StepTitle({title,desc}:{title:string;desc:string}) { return <div className="mb-7"><p className="text-sm font-black text-[#6877C8]">استكمال الملف</p><h2 className="mt-2 text-2xl font-black sm:text-3xl">{title}</h2><p className="mt-2 text-sm font-semibold leading-7 text-[#7D86A7]">{desc}</p></div>; }
