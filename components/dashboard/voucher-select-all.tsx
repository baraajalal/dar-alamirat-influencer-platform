...
export function VoucherSelectAll({ eligibleCount }: { eligibleCount: number }) {
  ...
  return (
    <label className="flex cursor-pointer items-center gap-2 rounded-xl border border-[#DDE3F5] bg-white px-4 py-3 text-sm font-black text-[#53618B]">
      <input
        type="checkbox"
        checked={checked}
        disabled={eligibleCount === 0}
        onChange={(event) => toggleAll(event.target.checked)}
        className="h-5 w-5 accent-[#596BC4] disabled:opacity-40"
      />
